...
def find_numpy_indices(A, B):
    len_a = A.shape[0]
    len_b = B.shape[0]
    potential_starts = np.where(B == A[0])[0]
    
    for start in potential_starts:
        if start + len_a <= len_b:
            if np.array_equal(B[start : start + len_a], A):
...
if __name__ == "__main__":
    ...
    # questions, labels = data_utils.load_datasets(malicious_only=False) # label 1 = refusal behavior
    questions, labels = data_utils.load_twinset() # label 1 = refusal behavior
    ...
    for batch_index, batch_prompts in enumerate(tqdm(data_utils.batchify(prompts, batch_size), total=total_batches)):
        ...
        for prompt_index, prompt in enumerate(input_tokens):
            input_tokens_question = tokenizer(questions[(batch_index*batch_size) + prompt_index], return_tensors="np", add_special_tokens=False, padding=False, truncation=False)["input_ids"][0]
            start, end = find_numpy_indices(input_tokens_question, prompt)
            
            # print("-----------------------------------------------------")
            # print(batch_prompts[prompt_index])
            # print(questions[(batch_index*batch_size) + prompt_index])
            # print("-----------------------------------------------------")
            # print(input_tokens[prompt_index].shape)
            # print(input_tokens_question.shape)
            # print("-----------------------------------------------------")
            # print(input_tokens[prompt_index])
            # print(input_tokens_question)
            # print(input_tokens[prompt_index][start:end])
            # print("-----------------------------------------------------")
            
            for layer_name in top_k_expert_indices:
                filtered_expert_choices_per_token[layer_name].append(top_k_expert_indices[layer_name][batch_index][prompt_index][start:end])
    ...
    data_utils.create_directory(BASE_FOLDER + f"gate_output/{model_config.model_name}")

    data_utils.save_data(filtered_expert_choices_per_token, BASE_FOLDER + f"gate_output/{model_config.model_name}/{model_config.model_name}_expert_indices.pkl")
